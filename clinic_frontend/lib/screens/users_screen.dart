...
const _roles = ['Admin', 'Doctor', 'Receptionist', 'Pharmacist', 'Diagnosist'];
  ...
  Widget build(BuildContext context) {
    ...
    if (user == null)
      return Scaffold(
          ...
          body: const AppEmptyView(message: 'Not logged in.'));
            ...
            AppCard(
              ...
              child: Column(children: [
                ...
                if (spec != null && spec.isNotEmpty) ...[
                  _divider(),
                  _InfoTile(
                      Icons.medical_services_outlined, 'Specialization', spec),
                ],
                _divider(),
                _InfoTile(
                  ...
                  valueColor:
                      isActive ? const Color(0xFF16A34A) : AppColors.statusRed,
                ),
              ]),
            ),
          ]),
...
class _ProfileForm extends StatefulWidget {
  ...
  const _ProfileForm(
      {required this.user, required this.clinicId, required this.onSaved});
  @override
...
class _ProfileFormState extends State<_ProfileForm> {
  ...
  final List<_Qualification> _qualifications = [];
  final List<String> _qualErrors = [];
  ...
  void initState() {
    ...
    // Load existing qualifications if any
    final quals = u['qualifications'];
    if (quals is List) {
      for (final q in quals) {
        _qualifications.add(_Qualification(
          qual: q['qualification'] ?? '',
          inst: q['institution'] ?? '',
        ));
      }
    }
    if (_qualifications.isEmpty && _role == 'Doctor') {
      _qualifications.add(_Qualification());
    }
  }
  ...
  void dispose() {
    ...
    _dobCtrl.dispose();
    _specCtrl.dispose();
    for (final q in _qualifications) q.dispose();
    super.dispose();
  }

  int _computeAge(String dobStr) {
    try {
      final dob = DateTime.parse(dobStr.substring(0, 10));
  ...
  bool _validate() {
    ...
    // VALIDATION: full name required; letters + spaces only; min 2 chars
    final name = _nameCtrl.text.trim().toString();
    if (name.isEmpty) {
      e['name'] = 'Required';
    } else if (name.length < 2) {
      e['name'] = 'Name must be at least 2 characters';
    } else if (!RegExp(r'^[A-Za-z .]+$').hasMatch(name)) {
      e['name'] = 'Only letters, spaces, and dots allowed';
    } else if (name.startsWith(' ')) {
      e['name'] = 'Name cannot start with a space';
    }
    ...
    // VALIDATION: blood_group required
    if (_bloodGroup == null) e['blood'] = 'Required';

    // VALIDATION: DOB required; not in future;
    //             Doctor must be > 24 yrs; non-Doctor > 18 yrs
    if (_dobCtrl.text.isEmpty) {
      ...
      if (age < 0) {
        ...
        final now = DateTime.now();
        final dob = DateTime.tryParse(_dobCtrl.text.substring(0, 10));
        if (dob != null && dob.isAfter(now)) {
          ...
          e['dob'] = 'Doctor must be at least $_minAgeDoctor years old';
            ...
            _role != 'Doctor' &&
            age < _minAgeNonDoctor) {
          e['dob'] = 'Must be at least $_minAgeNonDoctor years old';
        }
      }
    }

    // VALIDATION: specialization required for Doctor (field hidden for other roles)
    if (_role == 'Doctor' && _specCtrl.text.trim().isEmpty) {
      e['spec'] = 'Required for Doctor';
    }

    // ── STEP 3: Qualifications (Doctor only) ───────────────────
    if (_role == 'Doctor') {
      // VALIDATION: at least one qualification required for Doctor
      if (_qualifications.isEmpty) {
        e['quals'] = 'At least one qualification is required for Doctor';
      } else {
        final qualNames = <String>[];
        for (int i = 0; i < _qualifications.length; i++) {
          final qName = _qualifications[i].qualCtrl.text.trim();
          final iName = _qualifications[i].instCtrl.text.trim();

          if (qName.isEmpty) {
            e['qual_$i'] = 'Qualification name is required';
          } else {
            // VALIDATION: duplicate qualification name check
            final lowerName = qName.toLowerCase();
            if (qualNames.contains(lowerName)) {
              e['qual_$i'] = 'Duplicate qualification — already added';
            } else {
              qualNames.add(lowerName);
            }
          }

          // VALIDATION: institution name min 3 chars
          if (iName.isNotEmpty && iName.length < 3) {
            e['inst_$i'] = 'Institution name must be at least 3 characters';
          } else if (iName.isEmpty) {
            e['inst_$i'] = 'Institution is required';
          }
        }
      }
    }
  ...
  Widget _qualificationsSection() {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const Expanded(
            child: Text('Qualifications *',
                style: TextStyle(
                    ...
                    color: AppColors.textPrimary))),
        GestureDetector(
          onTap: () => setState(() => _qualifications.add(_Qualification())),
          child: Container(
      ...
      if (_errors['quals'] != null) _errText(_errors['quals']!),
        ...
        return Container(
          ...
          child:
            ...
            Row(children: [
              ...
              const Spacer(),
              if (_qualifications.length > 1)
                GestureDetector(
                  onTap: () => setState(() {
                    _qualifications[i].dispose();
                    _qualifications.removeAt(i);
                  }),
            ...
            const SizedBox(height: 8),
            // VALIDATION: qualification name required; duplicate check
            AppFormField(
                label: 'Qualification Name *',
                controller: q.qualCtrl,
                hint: 'e.g. MBBS, MD',
                onChanged: (_) => setState(() => _errors.remove('qual_$i'))),
            if (_errors['qual_$i'] != null) _errText(_errors['qual_$i']!),
            const SizedBox(height: 8),
            // VALIDATION: institution name required; min 3 chars
            AppFormField(
                label: 'Institution *',
                controller: q.instCtrl,
                hint: 'e.g. AIIMS Delhi',
                onChanged: (_) => setState(() => _errors.remove('inst_$i'))),
            if (_errors['inst_$i'] != null) _errText(_errors['inst_$i']!),
          ]),
        );
      }),
    ]);
        ...
        child: Column(children: [
          ...
          Expanded(
            ...
            child:
              ...
              // VALIDATION: name required; letters+spaces only; min 2 chars; no leading space
              AppFormField(
                  label: 'Full Name *',
              ...
              Row(children: [
                ...
                Expanded(
                    child: Column(
                      ...
                      if (_errors['role'] != null) _errText(_errors['role']!),
                    ])),
                const SizedBox(width: 12),
                // VALIDATION: blood_group required (was not validated before)
                Expanded(
                    child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                      ...
                      if (_errors['blood'] != null) _errText(_errors['blood']!),
                    ])),
              ]),
              const SizedBox(height: 14),

              // VALIDATION: DOB required; not future; Doctor ≥24 yrs; non-Doctor ≥18 yrs
              ...
              if (_errors['dob'] != null) _errText(_errors['dob']!),
              ...
              // VALIDATION: Specialization required ONLY for Doctor; field hidden for other roles
              if (_role == 'Doctor') ...[
                const SizedBox(height: 14),
                AppFormField(
                ...
                if (_errors['spec'] != null) _errText(_errors['spec']!),
              ],

              // ── SECTION 3: Qualifications (Doctor only) ──────────
              // VALIDATION: at least one qualification; no duplicates; institution min 3 chars
